...
public class cardlayout extends JFrame implements ActionListener
{
	...
	public cardlayout()
	{
		...
		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);//swing的窗体关闭很简单。
		...
		cardPanel.setLayout(card); //循环，在cardPanel面板对象中添加五个按钮
		// 因为cardPanel面板对象为卡片布局，因此只显示最先添加的组件

		for (int i = 0; i < 5; i++)
		{

			cardPanel.add(new TextField("文本" + i));
		...
		//在text2中显示鼠标在面板中的坐标。
		text3.addMouseMotionListener(new MouseMotionAdapter()//adapter好处就是用不到的方法不用被强制重写出来。
	
		{@Override
		public void mouseMoved(MouseEvent e)
		{
			// TODO Auto-generated method stub
			text2.setText(e.getX()+":"+e.getY());
			text3.setText(e.getX()+":"+e.getY());
		}
		
		});
		controlpaPanel.add(preButton);
		controlpaPanel.add(nextbutton);
		controlpaPanel.add(text2);
		cardPanel.add(text3);
		// 定义容器对象为当前窗体容器对象
